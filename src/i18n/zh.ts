...
export const messages: Record<string, string> = {
  ...
  'cmd.reply_mode.status': '当前群回复模式：{mode}\n可用命令：/reply-mode chat｜topic｜new-topic（shared 仍作为 topic 的兼容别名）',
  ...
  'cmd.reply_mode.usage': '用法：/reply-mode chat｜topic｜new-topic\nchat=群内平铺回复且复用群会话；topic=回复落在话题里但复用同一个群 session（shared 是兼容别名）；new-topic=每次 @ 都开一个独立话题+独立会话。',
